...
3. **Multi-Schema 並走** — `journal-mcp-canonical-v1` / `madr-v1` / `minimal-v1` 等を同一 project 内に混在可能、 chapter ごとに `chapter_meta.schema_id` で固定
...
- **`journal_import` migration tool** — imports an existing `journal-mcp-canonical-v1` markdown file
...
│   │   ├── embed/
│   │   │   ├── journal_mcp_canonical_v1.yaml  # ST2: built-in canonical journal schema (compile-time embed)
